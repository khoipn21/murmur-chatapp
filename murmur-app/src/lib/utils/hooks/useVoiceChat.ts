...
export function useSetupVoiceChat(guildId: string): void {
	...
	const [voiceClients, setVoiceClients] = voiceStore((state) => [
		state.voiceClients,
	...
	const [getConnection, setConnection, deleteConnection] = voiceStore(
		(state) => [
			state.getConnection,
			state.setConnection,
		...
		const onUserJoin = async (): Promise<void> => {
			...
			voiceClients.forEach((user) => {
				...
				if (!getConnection(user.id)) {
					...
					getConnection(user.id)!.onicecandidate = (
					...
					localStream
						?.getAudioTracks()
						.forEach((track) =>
							getConnection(user.id)!.addTrack(track, localStream!),
						);
				}
			});
			...
			if (voiceJoinUserId !== current?.id) {
				try {
					const description = await getConnection(
						voiceJoinUserId,
					)?.createOffer();

					await getConnection(voiceJoinUserId)?.setLocalDescription(
						description,
					);

					sendVoiceSignal({
						userId: voiceJoinUserId,
						sdp: getConnection(voiceJoinUserId)?.localDescription,
					});
				} catch (err) {}
			}
		};
	...
	useEffect((): void => {
		const onMessageFromServer = async (): Promise<void> => {
			...
			if (userId !== current?.id) {
				if (sdp) {
					try {
						await getConnection(userId)!.setRemoteDescription(
						...
						if (sdp?.type === "offer") {
							...
							sendVoiceSignal({
								userId,
								sdp: getConnection(userId)!.localDescription,
							});
						}
					} catch (err) {}
				}
				...
				if (ice) {
					try {
						await getConnection(userId)!.addIceCandidate(
