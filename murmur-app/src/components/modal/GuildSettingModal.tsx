...
} from "@api/handler/guild";
import CropImageModal from "./CropImageModal";
import { getBanList, unbanMember } from "@api/handler/member";
...
function GuildSettingModal({ guildId, isOpen, onClose }: IProps) {
...
function BanListModal({ goBack, guildId }: IBanScreenProps) {
	const key = ["bans", guildId];
	const { data } = useQuery({
		queryKey: key,
		queryFn: () => getBanList(guildId).then((response) => response.data),
	});
...
export default GuildSettingModal;
