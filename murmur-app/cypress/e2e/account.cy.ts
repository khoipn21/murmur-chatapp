...
describe("Account related pages", () => {
	...
	it("registers the user", () => {
		cy.mailslurp()
			...
			.then((inbox) => {
				...
				cy.registerUser(emailAddress, id);

				// Wait for the verification email
				cy.mailslurp()
					.then((mailslurp) =>
						mailslurp.waitForLatestEmail(inbox.id, 120000, true),
					)
					.then((email) => {
						const verificationLink = email.body.match(/href="([^"]*)/)[1];
	...
	it("signs in the user", () => {
		cy.mailslurp()
			.then((mailslurp) => mailslurp.getAllInboxes())
			.then((inboxes) => {
				const verifiedEmail = inboxes[0].emailAddress;
				cy.loginUser(verifiedEmail);
			});
	});
